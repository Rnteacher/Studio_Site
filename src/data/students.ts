export interface StudentContact {
  email: string;
  phone: string;
  socials?: {
    instagram?: string;
    facebook?: string;
    tiktok?: string;
  };
}

export interface Student {
  id: string;
  name: string;
  shortDescription: string;
  longDescription: string;
  image: string;
  categories: string[];
  services: Record<string, string[]>;
  contact: StudentContact;
}

export const students: Student[] = [
  {
    id: "miyael",
    name: "מיאל",
    shortDescription: "מוזיקאית רב-תחומית, יוצרת תכשיטים ומאמנת כוח וקונג פו",
    longDescription: "מיאל היא מוזיקאית מגוונת המנגנת גיטרה, שרה וכותבת שירים מקוריים. לצד המוזיקה, היא עוסקת בתכשיטנות יצירתית ומלמדת קונג פו ואימוני כוח. היא מאמינה בשילוב בין אמנות לגוף ומביאה גישה ייחודית לכל פרויקט.",
    image: "/images/miyael.jpeg",
    categories: ["מוזיקה", "אמנות", "ספורט ותנועה"],
    services: {
      "מוזיקה": ["נגינה בגיטרה", "שירה", "כתיבת שירים"],
      "אמנות": ["תכשיטנות"],
      "ספורט ותנועה": ["קונג פו", "אימוני כוח"],
    },
    contact: { email: "miyael@studio-dorian.co.il", phone: "050-0000001" },
  },
  {
    id: "emma",
    name: "אמה",
    shortDescription: "מנהיגה, זמרת, כותבת ויוצרת קרושה",
    longDescription: "אמה משלבת בין כישורי מנהיגות לכישרון אמנותי. היא מנהלת צוותים, כותבת טקסטים ושירים, שרה ויוצרת עבודות קרושה ייחודיות. היא מביאה אנרגיה חיובית ומקצועיות לכל פרויקט.",
    image: "/placeholder.svg",
    categories: ["מוזיקה", "אמנות", "תפירה וסריגה"],
    services: {
      "מוזיקה": ["שירה", "כתיבה יצירתית"],
      "אמנות": ["ניהול צוות"],
      "תפירה וסריגה": ["קרושה"],
    },
    contact: { email: "emma@studio-dorian.co.il", phone: "050-0000002" },
  },
  {
    id: "moriah",
    name: "מוריה",
    shortDescription: "אמנית, מעצבת גרפית, מאפרת ובשלנית יצירתית",
    longDescription: "מוריה היא יוצרת ויזואלית רב-תחומית. היא מציירת, מעצבת גרפיקה, עוסקת באופנה ואיפור, ואוהבת לבשל ולאפות. היא מביאה עין אמנותית לכל דבר שהיא נוגעת בו.",
    image: "/images/moriah.png",
    categories: ["אמנות", "בישול ואפייה"],
    services: {
      "אמנות": ["ציור", "עיצוב גרפי", "אופנה", "איפור"],
      "בישול ואפייה": ["בישול"],
    },
    contact: { email: "moriah@studio-dorian.co.il", phone: "050-0000003" },
  },
  {
    id: "noga",
    name: "נוגה",
    shortDescription: "תופרת, יוצרת תכשיטים, מדריכת סיורים ובשלנית",
    longDescription: "נוגה יוצרת ביצירתיות ובאהבה. היא תופרת בגדים ואקססוריז, מעצבת תכשיטים, מדריכה סיורים ייחודיים ומבשלת ארוחות מיוחדות. היא מחברת בין מסורת לחדשנות.",
    image: "/placeholder.svg",
    categories: ["תפירה וסריגה", "אמנות", "בישול ואפייה"],
    services: {
      "תפירה וסריגה": ["תפירה"],
      "אמנות": ["תכשיטנות", "הדרכת סיורים"],
      "בישול ואפייה": ["בישול"],
    },
    contact: { email: "noga@studio-dorian.co.il", phone: "050-0000004" },
  },
  {
    id: "tzvi",
    name: "צבי",
    shortDescription: "מפיק מוזיקלי, מהנדס סאונד ועורך וידאו",
    longDescription: "צבי הוא הקול מאחורי הקלעים. הוא מפיק מוזיקה, מתכנן ובונה אולפנים, עורך וידאו ומנהל סאונד והגברה באירועים. הידע הטכני שלו משולב עם אוזן מוזיקלית מפותחת.",
    image: "/placeholder.svg",
    categories: ["מוזיקה", "עריכה והפקה"],
    services: {
      "מוזיקה": ["הפקה מוזיקלית", "סאונד והגברה"],
      "עריכה והפקה": ["עריכת וידאו", "בניית אולפנים"],
    },
    contact: { email: "tzvi@studio-dorian.co.il", phone: "050-0000005" },
  },
  {
    id: "ido",
    name: "עידו",
    shortDescription: "בנאי עץ מיומן – מתמחה בכלובים מעוצבים מעץ",
    longDescription: "עידו בונה כלובים ומבנים מעץ בעבודת יד מדויקת. הוא משלב בין פונקציונליות לאסתטיקה, ויוצר מוצרים ייחודיים ואיכותיים. כל פרויקט הוא מלאכת מחשבת.",
    image: "/placeholder.svg",
    categories: ["בנייה ונגרות"],
    services: {
      "בנייה ונגרות": ["בניית כלובים מעץ"],
    },
    contact: { email: "ido@studio-dorian.co.il", phone: "050-0000006" },
  },
  {
    id: "yaara",
    name: "יערה",
    shortDescription: "אמנית ויטראז׳ – יוצרת יצירות זכוכית צבעוניות",
    longDescription: "יערה מתמחה באמנות הוויטראז׳ ויוצרת חלונות ויצירות זכוכית צבעוניות ומרהיבות. כל עבודה שלה היא ייחודית ומשלבת בין מסורת עתיקה לעיצוב מודרני.",
    image: "/placeholder.svg",
    categories: ["אמנות"],
    services: {
      "אמנות": ["ויטראז׳"],
    },
    contact: { email: "yaara@studio-dorian.co.il", phone: "050-0000007" },
  },
  {
    id: "alon",
    name: "אלון",
    shortDescription: "בונה ריהוט ומעצב תלת מימד",
    longDescription: "אלון משלב בין נגרות מעשית לעיצוב דיגיטלי. הוא בונה ריהוט בהתאמה אישית ומעצב מודלים תלת ממדיים. הגישה שלו מחברת בין חומרים פיזיים לעולם הווירטואלי.",
    image: "/placeholder.svg",
    categories: ["בנייה ונגרות", "מחשבים וטכנולוגיה"],
    services: {
      "בנייה ונגרות": ["בניית ריהוט"],
      "מחשבים וטכנולוגיה": ["עיצוב תלת מימד"],
    },
    contact: { email: "alon@studio-dorian.co.il", phone: "050-0000008" },
  },
  {
    id: "binyamin",
    name: "בנימין",
    shortDescription: "עורך סרטונים מקצועי עם עין לפרטים",
    longDescription: "בנימין עורך סרטונים ברמה גבוהה. הוא מתמחה בעריכת תוכן לרשתות חברתיות, סרטוני תדמית ופרויקטים יצירתיים. הוא מביא סיפור ויזואלי מרתק לכל פרויקט.",
    image: "/placeholder.svg",
    categories: ["עריכה והפקה"],
    services: {
      "עריכה והפקה": ["עריכת סרטונים"],
    },
    contact: { email: "binyamin@studio-dorian.co.il", phone: "050-0000009" },
  },
  {
    id: "idan",
    name: "עידן",
    shortDescription: "מהנדס רובוטיקה ואלקטרוניקה צעיר ויצירתי",
    longDescription: "עידן בונה רובוטים ומערכות אלקטרוניות מאפס. הוא משלב בין תכנות, חומרה ויצירתיות כדי ליצור פתרונות טכנולוגיים חדשניים. התשוקה שלו לטכנולוגיה מניעה אותו ליצור דברים מדהימים.",
    image: "/images/idan.jpeg",
    categories: ["מחשבים וטכנולוגיה"],
    services: {
      "מחשבים וטכנולוגיה": ["רובוטיקה", "אלקטרוניקה"],
    },
    contact: { email: "idan@studio-dorian.co.il", phone: "050-0000010" },
  },
  {
    id: "maya",
    name: "מאיה",
    shortDescription: "אמנית אקרובטיקה אווירית – ביצועים מרהיבים בגובה",
    longDescription: "מאיה מתמחה באקרובטיקה אווירית ומופיעה בביצועים מרהיבים. היא משלבת כוח, גמישות וביטוי אמנותי בגובה רב. ההופעות שלה הן חוויה ויזואלית בלתי נשכחת.",
    image: "/placeholder.svg",
    categories: ["ספורט ותנועה"],
    services: {
      "ספורט ותנועה": ["אקרובטיקה אווירית"],
    },
    contact: { email: "maya@studio-dorian.co.il", phone: "050-0000011" },
  },
  {
    id: "bar", name: "בר", shortDescription: "תיאור קצר - בקרוב", longDescription: "תיאור מלא של בר יתעדכן בקרוב.",
    image: "/images/bar.jpeg", categories: ["ללא קטגוריה"], services: {},
    contact: { email: "bar@studio-dorian.co.il", phone: "050-0000012" },
  },
  {
    id: "finn", name: "פין", shortDescription: "תיאור קצר - בקרוב", longDescription: "תיאור מלא של פין יתעדכן בקרוב.",
    image: "/placeholder.svg", categories: ["ללא קטגוריה"], services: {},
    contact: { email: "finn@studio-dorian.co.il", phone: "050-0000013" },
  },
  {
    id: "eden", name: "עדן", shortDescription: "תיאור קצר - בקרוב", longDescription: "תיאור מלא של עדן יתעדכן בקרוב.",
    image: "/images/eden.jpeg", categories: ["ללא קטגוריה"], services: {},
    contact: { email: "eden@studio-dorian.co.il", phone: "050-0000014" },
  },
  {
    id: "shiri", name: "שירי", shortDescription: "תיאור קצר - בקרוב", longDescription: "תיאור מלא של שירי יתעדכן בקרוב.",
    image: "/placeholder.svg", categories: ["ללא קטגוריה"], services: {},
    contact: { email: "shiri@studio-dorian.co.il", phone: "050-0000015" },
  },
  {
    id: "oren", name: "אורן", shortDescription: "תיאור קצר - בקרוב", longDescription: "תיאור מלא של אורן יתעדכן בקרוב.",
    image: "/placeholder.svg", categories: ["ללא קטגוריה"], services: {},
    contact: { email: "oren@studio-dorian.co.il", phone: "050-0000016" },
  },
  {
    id: "noya", name: "נויה", shortDescription: "תיאור קצר - בקרוב", longDescription: "תיאור מלא של נויה יתעדכן בקרוב.",
    image: "/placeholder.svg", categories: ["ללא קטגוריה"], services: {},
    contact: { email: "noya@studio-dorian.co.il", phone: "050-0000017" },
  },
  {
    id: "naama", name: "נעמה", shortDescription: "תיאור קצר - בקרוב", longDescription: "תיאור מלא של נעמה יתעדכן בקרוב.",
    image: "/placeholder.svg", categories: ["ללא קטגוריה"], services: {},
    contact: { email: "naama@studio-dorian.co.il", phone: "050-0000018" },
  },
  {
    id: "aviv", name: "אביב", shortDescription: "תיאור קצר - בקרוב", longDescription: "תיאור מלא של אביב יתעדכן בקרוב.",
    image: "/placeholder.svg", categories: ["ללא קטגוריה"], services: {},
    contact: { email: "aviv@studio-dorian.co.il", phone: "050-0000019" },
  },
  {
    id: "yishai", name: "ישי", shortDescription: "תיאור קצר - בקרוב", longDescription: "תיאור מלא של ישי יתעדכן בקרוב.",
    image: "/placeholder.svg", categories: ["ללא קטגוריה"], services: {},
    contact: { email: "yishai@studio-dorian.co.il", phone: "050-0000020" },
  },
  {
    id: "rafael", name: "רפאל", shortDescription: "תיאור קצר - בקרוב", longDescription: "תיאור מלא של רפאל יתעדכן בקרוב.",
    image: "/placeholder.svg", categories: ["ללא קטגוריה"], services: {},
    contact: { email: "rafael@studio-dorian.co.il", phone: "050-0000021" },
  },
  {
    id: "hillel", name: "הלל", shortDescription: "תיאור קצר - בקרוב", longDescription: "תיאור מלא של הלל יתעדכן בקרוב.",
    image: "/images/hillel.jpeg", categories: ["ללא קטגוריה"], services: {},
    contact: { email: "hillel@studio-dorian.co.il", phone: "050-0000022" },
  },
  {
    id: "arbel", name: "ארבל", shortDescription: "תיאור קצר - בקרוב", longDescription: "תיאור מלא של ארבל יתעדכן בקרוב.",
    image: "/placeholder.svg", categories: ["ללא קטגוריה"], services: {},
    contact: { email: "arbel@studio-dorian.co.il", phone: "050-0000023" },
  },
  {
    id: "natali", name: "נטלי", shortDescription: "תיאור קצר - בקרוב", longDescription: "תיאור מלא של נטלי יתעדכן בקרוב.",
    image: "/placeholder.svg", categories: ["ללא קטגוריה"], services: {},
    contact: { email: "natali@studio-dorian.co.il", phone: "050-0000024" },
  },
  {
    id: "toby", name: "טובי", shortDescription: "תיאור קצר - בקרוב", longDescription: "תיאור מלא של טובי יתעדכן בקרוב.",
    image: "/placeholder.svg", categories: ["ללא קטגוריה"], services: {},
    contact: { email: "toby@studio-dorian.co.il", phone: "050-0000025" },
  },
  {
    id: "yoav", name: "יואב", shortDescription: "תיאור קצר - בקרוב", longDescription: "תיאור מלא של יואב יתעדכן בקרוב.",
    image: "/placeholder.svg", categories: ["ללא קטגוריה"], services: {},
    contact: { email: "yoav@studio-dorian.co.il", phone: "050-0000026" },
  },
];
